...
class Schema(object):
    ...
    def __init__(self, schema, error=None, ignore_extra_keys=False, name=''):
        self._schema = schema
        self._error = error
    ...
    def validate(self, data):
        ...
        if flavor == DICT:
            ...
            if not required.issubset(coverage):
                ...
                if custom_schema_name != '':
                    raise \
                        SchemaMissingKeyError("'" + custom_schema_name + "' " +
                                              'missing keys: ' +
                                              s_missing_keys, e)
                else:
                    raise \
                        SchemaMissingKeyError('Missing keys: ' +
                                              s_missing_keys, e)
            ...
            if isinstance(data, s) and not (isinstance(data, bool) and s == int):
                ...
                if custom_schema_name != '':
                    raise SchemaUnexpectedTypeError(
                        '%s: %r should be instance of %r' %
                        (custom_schema_name, data, s.__name__),
                        e.format(data) if e else None)
                else:
                    raise SchemaUnexpectedTypeError(
        ...
        if flavor == CALLABLE:
            ...
            try:
                ...
                if custom_schema_name != '':
                    raise SchemaError(
                        '%s: %s(%r) raised %r' % (custom_schema_name, f, data,
                                                  x),
                        self._error.format(data) if self._error else None)
                else:
                    raise SchemaError(
