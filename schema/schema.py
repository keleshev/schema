...
class Regex(object):
    ...
    def __init__(self, pattern_str, flags=0, error=None):
        ...
        flags_list = [
            Regex.NAMES[i]
            for i, f in enumerate("{:09b}".format(int(flags)))
            if f != "0"
        ]  # Name for each bit
    ...
    def validate(self, data, **kwargs):
        ...
        try:
            if self._pattern.search(data):
                ...
                raise SchemaError(
                    "{!r} does not match {!r}".format(self, data),
...
class Schema(object):
    ...
    def validate(self, data, **kwargs):
        ...
        if flavor == DICT:
            ...
            defaults = {
                k for k in s if isinstance(k, Optional) and hasattr(k, "default")
            } - coverage
...
class Forbidden(Hook):
    ...
    def _default_function(nkey, data, error):
        raise SchemaForbiddenKeyError(
            "Forbidden key encountered: {!r} in {!r}".format(nkey, data), error
        )
